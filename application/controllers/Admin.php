...
class Admin extends CI_Controller
{
	...
	public function proses_tambah_transaksi_barang($id_detail_produk)
	{
		...
		$getAPITransaksi = $this->curl->simple_get($this->api . 'transaksi/get_transaksi_belum_lunas_by_id_user/' . $this->session->userdata('id_user'));
	...
	public function tambah_transaksi_jasa($id_detail_produk)
	{
		date_default_timezone_set('Asia/Jakarta');
		$now = date('Y-m-d H:i:s');
		$getAPI = $this->curl->simple_get($this->api . 'katalogProduk/barang/' . $id_detail_produk);
	...
	public function stok_tambah($id_detail_transaksi)
	{
		...
		$data = array(
			...
			'total_transaksi' 	=> $data2['sub_total'],
		);
