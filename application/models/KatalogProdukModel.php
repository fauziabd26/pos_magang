...
class KatalogProdukModel extends CI_Model
{
	...
	public function by_id_user($id_owner, $id_detail_produk = null)
	{
		...
		$this->db->select('id_detail_produk, toko.id_toko, toko.nama_toko, user.id_user, produk.id_produk, produk.nama_produk, kategori.id_kategori, kategori.nama_kategori, satuan.id_satuan, satuan.nama_satuan, harga.id_harga, harga.nama_harga, detail_produk.nominal, produk.jenis');
		$this->db->from('detail_produk')
			->join('produk', 'detail_produk.id_produk = produk.id_produk')
			->join('toko', 'produk.id_toko = toko.id_toko')
			->join('user', 'toko.id_user = user.id_user')
	...
	public function by_id_toko($id_toko)
	{
		$this->db->where('detail_produk.id_toko', $id_toko);
		$this->db->select('id_detail_produk, toko.id_toko, toko.nama_toko, user.id_user, produk.id_produk, produk.nama_produk, kategori.id_kategori, kategori.nama_kategori, satuan.id_satuan, satuan.nama_satuan, harga.id_harga, harga.nama_harga, detail_produk.nominal, produk.jenis');
		$this->db->from('detail_produk')
			->join('produk', 'detail_produk.id_produk = produk.id_produk')
			->join('toko', 'produk.id_toko = toko.id_toko')
